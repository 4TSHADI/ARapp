...
public class Swirl : Piping
{
    ...
    private bool editingSwirl = false;
    ...
    private string selectedShapeLocalSwirl;
    ...
    public override void Generate()
    {
        ...
        selectedShapeLocalSwirl = "" + shapeAndTypeHandler.selectedShape;
    ...
    private void GenerateSwirlPiping()
    {
        freqVal = (int)freqSlider.value;
        heightVal = heightSlider.value;
        float angleStep = 360f / freqVal; // Angle step between each swirl
        ...
        for (int i = 0; i < freqVal; i++)
        {
            ...
            GameObject pipingObject = new GameObject("swirl_" + i);
            ...
            CreateSwirlShapes(basePosition, shapes);
    ...
    private void CreateSwirlShapes(Vector3 basePosition, List<Shape> shapes)
    {
        ...
        for (int i = 0; i < pipingResolution; i++)
        {
            ...
            // Calculate the position along the swirl relative to the base position
            ...
            // Swirl rotation for this segment
            ...
            if (selectedShapeLocalSwirl == "Star")
            {
                newShape = new Star();
                ((Star)newShape).Initialize(sides, radius, radius * (1 - depth), i, newPosition, rotation, isCap, isFlipped);
            }
            else if (selectedShapeLocalSwirl == "FrenchStar")
            {
                int sidesFrench = 80;
                newShape = new Star();
                ((Star)newShape).Initialize(sidesFrench, radius, i, newPosition, rotation, isCap, isFlipped);
            }
            else if (selectedShapeLocalSwirl == "Petal")
            {
                newShape = new Petal();
                ((Petal)newShape).Initialize(sides, radius, i, newPosition, rotation, isCap, isFlipped);
            }
            else
            {
                newShape = new Circle();
