...
public class Twisted : Piping
{
    private List<GameObject> createdObjects = new List<GameObject>();
    int pipingResolution = 20;
    int sides = 20;
    private Spline spline; // The actual spline
    public int numKnots = 64; // Number of knots to make the circle smoother
    private bool isGenerating = false;
    private Color newColor;
    private bool editingTwisted = false;
    private string selectedShapeLocalTwisted;
    public int twistTightness = 8;
    ...
    public override void Generate()
    {
        ...
        editingTwisted = true;
        selectedShapeLocalTwisted = "" + shapeAndTypeHandler.selectedShape;
        Debug.Log(selectedShapeLocalTwisted);


        // Generate the Twisted piping
        CreateCircularSpline();
        GenerateTwistedPiping();
    }
    ...
    public void deletePiping()
    {
        ...
        isGenerating = false;
        editingTwisted = false;

    }

    private void GenerateTwistedPiping()
    {
        ...
        GameObject pipingObject = new GameObject("TwistedPiping");
    ...
    //        if (selectedShapeLocalTwisted == "Star")
    //        {
    //            Debug.Log(selectedShapeLocalTwisted);
    //            Debug.Log("Selected Shape" + i);
    //            newShape = new Star();
    ...
    private void CreateCircularSpline()
    {
        ...
        spline.Clear();
        float angleStep = twistTightness * 360f / pipingResolution;
        ...
        // Create a circular spline with horizontal swirl
        for (int i = 0; i < numKnots; i++)
        {
            Debug.Log("position---" + pipingPosition);

            // Calculate the angle for the circular spline
            ...
            // Add horizontal swirl effect
            float swirlRadius = cakeRadius * 0.2f; // Adjust the swirl radius as needed
            float swirlAngle = i * angleStep; // Angle for the swirl effect
            Vector3 swirlOffset = new Vector3(
                 // No horizontal offset (swirl is vertical)
                swirlRadius * Mathf.Sin(Mathf.Deg2Rad * swirlAngle), // Vertical oscillation
                swirlRadius * Mathf.Cos(Mathf.Deg2Rad * swirlAngle)
                // Depth oscillation
            );

            // Adjust the knot position with the swirl effect
            Vector3 knotPositionAdjusted = knotPosition + swirlOffset;

            // Rotation for the knot
            Quaternion knotRotation = Quaternion.Euler(0, -(90 * i), 0);

            // Create the BezierKnot and add it to the spline
            BezierKnot knot = new BezierKnot(knotPositionAdjusted);
        ...
        GameObject splineObject = new GameObject("Spline_Twisted");
    ...
    private void CreateShapesAlongSpline(List<Shape> shapes)
    {
        ...
        for (int i = 0; i < pipingResolution; i++)
        {
            ...
            bool isCap = (i == 0 || i == pipingResolution - 1);
            bool isFlipped = (i == pipingResolution - 1);
            Debug.Log(selectedShapeLocalTwisted + "-----dfgjtfjkg" + depth);
